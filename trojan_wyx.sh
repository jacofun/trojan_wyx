...
EOF
	...
	cd /usr/share/nginx/
	wget https://github.com/jacofun/jacofun.github.io/archive/refs/tags/publish.zip >/dev/null 2>&1
    unzip publish.zip >/dev/null 2>&1
	mv jacofun.github.io-publish html >/dev/null 
	...
	~/.acme.sh/acme.sh --set-default-ca  --server letsencrypt #将默认CA改为letsencrypt，ZeroSSL经常超时
	~/.acme.sh/acme.sh --register-account -m noreply@jaco.fun 
	...
	#trojan_passwd=$(cat /dev/urandom | head -1 | md5sum | head -c 8) Trojan密码的生成方式
